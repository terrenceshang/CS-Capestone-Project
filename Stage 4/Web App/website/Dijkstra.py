...
class Dijkstra():
    ...
    def findRoute(self,start_node, end_node):
        graph, lstACRoute, lstANRoute, lstASRoute, StationsArea = self.createGraph()
        previous_nodes, shortest_path = self.dijkstra_algorithm(graph=graph, start_node = start_node)
        arrPath = self.print_result(previous_nodes, shortest_path, start_node=start_node, target_node= end_node)
        
        return arrPath

    def getDuration(self,graph, start_node, end_node):
        return graph.value(start_node, end_node)

    def checkRoute(self,graph, route, path, conTrain):
        matches = []
        routeName = ""
        #print(path)
        furthest = ""
            ...
            for linePath in path:                                   
                ...
                for i in lineRoute:
                    ...
                    if found == True and len(prev) > 0:
                        duration += self.getDuration(graph, prev, linePath) 
                    if i == linePath:
                        if len(conTrain) > 1 and linePath == path[0]:                        
                            if count > pos:
                                duration += self.getDuration(graph, conTrain, linePath)
                            else:
                                break
                        bPathFound = True 
                        found = True
        ...
        if matches.__len__() == 0:
            ...
            for repeat in range(2):
                if repeat == 0:
                    matches.append(self.checkRoute(graph, route, path[0:count +1], conTrain))
                else:
                    conTrain = ""
                    matches.append(self.checkRoute(graph, route, path[count:len(path)], conTrain))
        return matches
